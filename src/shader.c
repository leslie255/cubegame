...
static inline GLuint compile_shader(usize length, const char source[length],
                                    GLenum shader_type) {
  ...
  if (!success) {
    ...
    glGetShaderInfoLog(shader, sizeof(shader_info_log), &info_log_length,
                       shader_info_log);
...
static inline bool link_shader_program(GLuint program) {
  ...
  if (!success) {
    GLsizei log_len = 0;
    glGetShaderInfoLog(program, sizeof(shader_info_log), &log_len,
                       shader_info_log);
...
ShaderProgram shader_init(usize vs_src_len, const char vs_src[vs_src_len],
  ...
  GLuint fragment_shader =
      compile_shader(fs_src_len, fs_src, GL_FRAGMENT_SHADER);
...
void shader_use(ShaderProgram shader) { glUseProgram(shader.gl_handle); }
