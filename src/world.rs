...
pub struct World<'scope, 'res> {
    ...
    chunk_building_thread: Option<thread::ScopedJoinHandle<'scope, ()>>,
...
impl<'scope, 'res> World<'scope, 'res> {
    ...
    pub fn new(
        ...
        let mut self_ = Self {
            ...
            chunk_building_thread: None,
            chunk_builder: ChunkBuilder::new(resources),
        };
        self_.init_chunk_building_thread();
        self_
    }

    fn init_chunk_building_thread(&mut self) {
        let chunks = Arc::clone(&self.chunks);
        let chunk_builder = self.chunk_builder.clone();
        let thread = self.thread_scope.spawn(move || {
            _ = chunks;
            _ = chunk_builder;
            loop {
                println!("Chunk building thread says hi!");
                thread::sleep(Duration::from_secs(1));
            }
        });
        self.chunk_building_thread = Some(thread);
    }
    ...
    fn build_chunk(&mut self, chunk_id: ChunkId) {
        ...
        let chunk_mesh = self.chunk_meshes.get_mut(index).unwrap();
        self.chunk_builder.build(chunk, chunk_mesh);
    }
